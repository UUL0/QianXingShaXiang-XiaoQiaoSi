...
def 图元拟合静态实体(data_list,output_file):
    # 设定起始初始参数
    GUID起始值=1077943195 # 需要确保大于装饰GUID起始+图形数量+1
    ...
    内参[1]="      '02:"
    内参[2]=f"{内排序ID:02d}" # 内排序ID
    内参[3]=":embedded message': \n        {\n          '02:00:Varint': 1,\n          '03:01:Varint': 14,\n          '04:02:Varint': "
    内参[4]=f"{装饰GUID}" # 装饰GUID
    内参[5]="\n        },\n"
    ...
    for 当前生成ID in range(生成N): # 生成素材内容ID列
        装饰GUID=装饰起始GUID+1+当前生成ID
        内参[2]=f"{内排序ID:02d}" # 内排序ID
        内参[4]=f"{装饰GUID}" # 装饰GUID
        自身容器ID列.append(''.join(内参))
        内排序ID+=1

    当前生成ID=0

    参数[6] ="\n        },\n" +  ''.join(自身容器ID列) + "      '03:" + f"{内排序ID:02d}" + ":string': '"
    内排序ID+=1

    参数[7]=f"{实体名称}" # 名称
    参数[8]="',\n      '05:" + f"{内排序ID:02d}" + ":Varint': 1,\n      '11:" + f"{内排序ID:02d}" + ":embedded message': \n        {\n          '01:00:embedded message': \n            {\n              '01:00:Varint': "
    参数[9]=f"{GUID起始值}" # GUID
    ...
    for 当前生成ID in range(生成N): # 生成素材内容ID列
        ...
        if 当前生成ID == (生成N - 1): # 最后一次
            自身容器内容ID列表.append(f"{装饰GUID}")
        else:
            自身容器内容ID列表.append(f"{装饰GUID},")

    内参2[2]=''.join(自身容器内容ID列表)
    当前生成ID=0
    ...
    参数[42]=",\n                      '03:02:Varint': "
    参数[43]="4294901760" # 引用ARGB
    参数[44]=",\n                      '04:03:32-bit': "
    参数[45]="100.0" # 透明度
    参数[46]=",\n                      '05:04:Varint': "
    参数[47]="16711680" # 实体RGB
    参数[48]=",\n                      '06:05:Varint': 6700\n                    }\n                },\n              '10:25:Varint': 1\n            }\n        }\n    },\n"
    ...
    装饰参[1]="  '02:"
    装饰参[2]="" # 排序ID
    装饰参[3]=":embedded message': \n    {\n      '01:00:embedded message': \n        {\n          '02:00:Varint': 1,\n          '03:01:Varint': 14,\n          '04:02:Varint': "
    装饰参[4]="" # 装饰GUID
    装饰参[5]="\n        },\n      '03:01:string': '"
    装饰参[6]="" # 装饰名称
    装饰参[7]="',\n      '05:02:Varint': 28,\n      '21:03:embedded message': \n        {\n          '01:00:embedded message': \n            {\n              '01:00:Varint': "
    装饰参[8]="" # 装饰GUID
    装饰参[9]=",\n              '02:01:Varint': "
    ...
    装饰参[16]=f"{起始坐标X}" # 起始坐标X
    装饰参[17]=",\n                          '02:01:32-bit': "
    装饰参[18]=f"{起始坐标Y}" # 起始坐标Y

    装饰参[19]=",\n                          '03:02:32-bit': "
    装饰参[20]=f"{起始坐标Z}" # 起始坐标Z
    装饰参[21]="\n                        },\n                      '02:01:embedded message': \n                        {\n                          '01:00:32-bit': "
    装饰参[22]=f"{实体旋转X}" # 实体旋转X
    装饰参[23]=",\n                          '02:01:32-bit': "
    装饰参[24]=f"{实体旋转Y}" # 实体旋转Y
    装饰参[25]=",\n                          '03:02:32-bit': "
    装饰参[26]=f"{实体旋转Z}" # 实体旋转Z
    装饰参[27]="\n                        },\n                      '03:02:embedded message': \n                        {\n                          '01:00:32-bit': "
    装饰参[28]=f"{实体缩放X}" # 实体缩放X
 
    装饰参[29]=",\n                          '02:01:32-bit': "
    装饰参[30]=f"{实体缩放Y}" # 实体缩放Y
    装饰参[31]=",\n                          '03:02:32-bit': "
    装饰参[32]=f"{实体缩放Z}" # 实体缩放Z
    ...
    for 序号 in range(总元数量):
        ...
        装饰参[2]=f"{排序ID:02d}" # 排序ID
        装饰参[4]=f"{装饰GUID}" # 装饰GUID
        装饰参[6]=f"{装饰名称}_{序号}" # 装饰名称
        装饰参[8]=f"{装饰GUID}" # 装饰GUID
        装饰参[10]=f"{图形ID}" # 图形ID
        装饰参[12]=f"{装饰名称}_{序号}" # 装饰名称
        装饰参[14]=f"{GUID起始值}" # 元件容器GUID
        装饰参[16]=f"{实体位置X}" # 起始坐标X
        装饰参[18]=f"{实体位置Y}" # 起始坐标Y
        装饰参[20]=f"{实体位置Z}" # 起始坐标Z
        装饰参[22]=f"{实体旋转X}" # 实体旋转X
        装饰参[24]=f"{实体旋转Y}" # 实体旋转Y
        装饰参[26]=f"{实体旋转Z}" # 实体旋转Z
        装饰参[28]=f"{实体缩放X}" # 实体缩放X
        ...
        装饰参[34]=f"{装饰碰撞}" # 碰撞
        装饰参[36]=f"{装饰攀爬}" # 攀爬
        装饰参[38]=f"{引用ARGB}" # ARGB
        装饰参[40]=f"{实体颜色透明度}" # 透明度
        装饰参[42]=f"{实体颜色}" # RGB


        实体循环结果="".join(装饰参) # 拼接数组
        buffer.write(实体循环结果)
        排序ID+=1
