...
REM mkdir "批量封装GIA" 2>nul

REM for %%f in (%*) do (
REM     set "input=%%~f"
REM     set "output=批量封装GIA\%%~nf_封装.gia"
REM     echo 正在封装：%1
REM     py 合并封装流程.py "!input!" "!output!" || echo 处理失败: "!input!"
REM )
REM pause
...
set "output=批量封装元件gia\%filename_no_ext%_封装.gia"

REM 执行 Python 脚本
REM 确保 python 脚本路径正确，如果不在同一目录，请修改下方路径
py "合并封装流程.py" "!input!" "!output!"
