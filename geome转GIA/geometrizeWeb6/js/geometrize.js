...
var GeometrizeWorkerInterface = function() {
	var _gthis = this;
	this.worker = new Worker("geometrize_worker.js");
...
Main.prototype = {
	...
	,onWindowLoaded: function() {
		...
		Main.isoscelesTrianglesCheckbox.addEventListener("click",function() {
			setShapeOption(8,Main.isoscelesTrianglesCheckbox.checked);
		},false);
...
geometrize_exporter_ShapeJsonExporter.exportShape = function(shape) {
	...
	if(type === 8) {
		result += ",\n        \"params\":" + "[";
		var params_arr = shape.shape.getParams();
		var _gp = 0;
		var _gp1 = params_arr.length;
		while(_gp < _gp1) {
			var pi = _gp++;
			result += params_arr[pi];
			if(pi <= params_arr.length - 2) { result += ","; }
		}
		result += "],\n";
		result += "        \"score\":" + score + "\n";
	} else {
...
geometrize_shape_ShapeFactory.create = function(type,xBound,yBound) {
	...
	case 8:
		return new geometrize_shape_IsoscelesTriangle(xBound,yBound);
	}
};
...
var geometrize_shape_IsoscelesTriangle = function(xBound,yBound) {
	this.gx = Std.random(xBound);
	this.gy = Std.random(yBound);
	this.halfBase = 8 + Math.floor(25 * Math.random());
	this.height = 8 + Math.floor(25 * Math.random());
	this.angle = Math.random() * Math.PI * 2;
	this.xBound = xBound;
	this.yBound = yBound;
};
geometrize_shape_IsoscelesTriangle.__name__ = true;
geometrize_shape_IsoscelesTriangle.prototype = {
	_getVertices: function() {
		var cos = Math.cos(this.angle);
		var sin = Math.sin(this.angle);

		var halfH = this.height / 2;
		// 底边中点：从高中点沿高反方向（指向底边）偏移 h/2
		var mx = this.gx + halfH * sin;
		var my = this.gy - halfH * cos;

		// 底边中点：重心沿高方向（高指向底边方向）偏移 h/3
		//var mx = this.gx + (this.height / 3) * sin;
		//var my = this.gy - (this.height / 3) * cos;

		// 底边两端点
		var x1 = (mx - this.halfBase * cos) | 0;
		var y1 = (my - this.halfBase * sin) | 0;
		var x2 = (mx + this.halfBase * cos) | 0;
		var y2 = (my + this.halfBase * sin) | 0;

		// 顶角顶点：从高中点沿高方向偏移 h/2
		var x3 = (this.gx - halfH * sin) | 0;
		var y3 = (this.gy + halfH * cos) | 0;

		//// 底边两端点
		//var x1 = (mx - this.halfBase * cos) ;
		//var y1 = (my - this.halfBase * sin) ;
		//var x2 = (mx + this.halfBase * cos) ;
		//var y2 = (my + this.halfBase * sin) ;

		//// 顶角顶点：从高中点沿高方向偏移 h/2
		//var x3 = (this.gx - halfH * sin) ;
		//var y3 = (this.gy + halfH * cos) ;


		// 顶角顶点：重心沿高反方向偏移 2h/3
		//var x3 = (this.gx + (2 * this.height / 3) * (-sin)) | 0;
		//var y3 = (this.gy + (2 * this.height / 3) * cos) | 0;

		return [{x:x1,y:y1},{x:x2,y:y2},{x:x3,y:y3}];
	}
	,rasterize: function() {
		var v = this._getVertices();
		return geometrize_rasterizer_Scanline.trim(
			geometrize_rasterizer_Rasterizer.scanlinesForPolygon(v),
	...
	,mutate: function() {
		...
		switch(r) {
		case 0:
			value = this.gx + (-16 + Math.floor(33 * Math.random()));
			max = this.xBound - 1;
			this.gx = value < 0 ? 0 : value > max ? max : value;
			value = this.gy + (-16 + Math.floor(33 * Math.random()));
			max = this.yBound - 1;
			this.gy = value < 0 ? 0 : value > max ? max : value;
			break;
		case 1:
			value = this.halfBase + (-8 + Math.floor(17 * Math.random()));
			this.halfBase = value < 2 ? 2 : value > 200 ? 200 : value;
			break;
		case 2:
			value = this.height + (-8 + Math.floor(17 * Math.random()));
			this.height = value < 2 ? 2 : value > 200 ? 200 : value;
			break;
		case 3:
			this.angle += (-0.2 + 0.4 * Math.random());
			// 规范到 [0, 2π)
			var twoPI = Math.PI * 2;
	...
	,clone: function() {
		var t = new geometrize_shape_IsoscelesTriangle(this.xBound,this.yBound);
		t.gx = this.gx;
		t.gy = this.gy;
		t.halfBase = this.halfBase;
	...
	,getParams: function() {
		return [this.gx | 0, this.gy | 0, this.halfBase * 2, this.height, this.angle * (180 / Math.PI)];
	}
	,getSvgShapeData: function() {
		var v = this._getVertices();
		return "<polygon points=\"" + v[0].x + "," + v[0].y + " " + v[1].x + "," + v[1].y + " " + v[2].x + "," + v[2].y + "\" " + geometrize_exporter_SvgExporter.SVG_STYLE_HOOK + "/>";
	}
};
geometrize_shape_IsoscelesTriangle.__name__ = true;
...
Main.isoscelesTrianglesCheckbox = window.document.getElementById("isoscelestriangles");
...
geometrize_shape_ShapeTypes.ISOSCELES_TRIANGLE = 8;
Main.main();
